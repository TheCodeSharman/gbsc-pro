...
TEST_CASE("a source the key cannot identify is not stored")
{
    ...
    CHECK_FALSE(table.remember(SourceKey(97, 50.08f, 0.0f, SourceKey::Negative, SourceKey::Negative), Framed));
    CHECK(table.count() == 0);
}
...
{
    FramingTable table;
    const SourceKey sixty(311, 60.05f, 0.0f, SourceKey::Negative, SourceKey::Negative);
...
TEST_CASE("a full table refuses rather than discarding a tuning")
{
    ...
    for (uint16_t i = 0; i < FramingTable::Entries; ++i)
        REQUIRE(table.remember(SourceKey((uint16_t)(200 + i), 50.0f, 0.0f, SourceKey::Negative, SourceKey::Negative), Framed));

    CHECK(table.count() == FramingTable::Entries);
    CHECK_FALSE(table.remember(SourceKey(900, 50.0f, 0.0f, SourceKey::Negative, SourceKey::Negative), Framed));

    // And every earlier tuning is still there.
    PanAndZoom found;
    CHECK(table.find(SourceKey(200, 50.0f, 0.0f, SourceKey::Negative, SourceKey::Negative), &found));
}
...
TEST_CASE("a full table still takes a re-tune of a source already in it")
{
    ...
    const PanAndZoom retuned(0.1f, 0.5f, 0.2f, 0.6f);
    CHECK(table.remember(SourceKey(200, 50.0f, 0.0f, SourceKey::Negative, SourceKey::Negative), retuned));
}
...
TEST_CASE("a refused change does not move the revision")
{
    ...
    CHECK_FALSE(table.forget(SourceKey(0, 0.0f, 0.0f, SourceKey::Negative, SourceKey::Negative)));
    CHECK(table.revision() == stored);
}
...
TEST_CASE("a full table refuses a new source without moving the revision")
{
    ...
    CHECK_FALSE(table.remember(SourceKey(999, 50.0f, 0.0f, SourceKey::Negative, SourceKey::Negative), Framed));
