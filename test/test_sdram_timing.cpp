// Host-compiled unit tests for src/tv5725/SdramTiming.h -- `make -C test sdram-timing`.
//
...
TEST_CASE("a cycle-count register value is two clocks more than it reads")
{
    ...
    CHECK(SdramTiming::cyclesFor(0) == 2);
    CHECK(SdramTiming::cyclesFor(ThreeClocks) == 3);
    CHECK(SdramTiming::cyclesFor(2) == 4);
    CHECK(SdramTiming::cyclesFor(3) == 5);
}

TEST_CASE("the divider's codes are the frequencies the datasheet lists")
{
    CHECK(SdramTiming::clockKHz(0) == 108000u);
    CHECK(SdramTiming::clockKHz(1) == 81000u);
    CHECK(SdramTiming::clockKHz(3) == 162000u);
    CHECK(SdramTiming::clockKHz(4) == 144000u);
    CHECK(SdramTiming::clockKHz(5) == 185143u);
    CHECK(SdramTiming::clockKHz(6) == 216000u);
    CHECK(SdramTiming::clockKHz(7) == 129600u);
}
...
TEST_CASE("FBCLK names a pin, not a frequency, so it can never be derived")
{
    ...
    CHECK(SdramTiming::clockKHz(SdramTiming::FbclkCode) == 0u);
    CHECK_FALSE(SdramTiming::inSpec(SdramTiming::FbclkCode, ThreeClocks, ThreeClocks));
    CHECK(SdramTiming::fastestInSpec() != SdramTiming::FbclkCode);
    // And there is nothing to program alongside it either.
    CHECK(SdramTiming::actCycleRegister(SdramTiming::FbclkCode)
          == SdramTiming::NotEncodable);
}
...
TEST_CASE("162MHz is in spec and 185MHz is not, which is the whole decision")
{
    ...
    CHECK(SdramTiming::inSpec(3, ThreeClocks, ThreeClocks));
    ...
    // the argument.
    CHECK_FALSE(SdramTiming::inSpec(5, ThreeClocks, ThreeClocks));
    CHECK_FALSE(SdramTiming::inSpec(6, ThreeClocks, ThreeClocks));
}

TEST_CASE("everything at or below 162MHz is in spec, including what shipped")
{
    CHECK(SdramTiming::inSpec(1, ThreeClocks, ThreeClocks));  // 81
    CHECK(SdramTiming::inSpec(0, ThreeClocks, ThreeClocks));  // 108
    CHECK(SdramTiming::inSpec(7, ThreeClocks, ThreeClocks));  // 129.6, was shipping
    CHECK(SdramTiming::inSpec(4, ThreeClocks, ThreeClocks));  // 144
}

TEST_CASE("the fastest clock the part allows is 162MHz")
{
    CHECK(SdramTiming::fastestInSpec() == 3);
}
...
TEST_CASE("the cycle counts are computed from the clock, not inherited")
{
    ...
    CHECK(SdramTiming::cyclesNeeded(3, SdramTiming::TrcdMinPs) == 3);
    CHECK(SdramTiming::cyclesNeeded(3, SdramTiming::TrpMinPs) == 3);
    CHECK(SdramTiming::actCycleRegister(3) == ThreeClocks);
    CHECK(SdramTiming::pchgCycleRegister(3) == ThreeClocks);
    ...
    CHECK(SdramTiming::cyclesNeeded(1, SdramTiming::TrcdMinPs) == 2);
    CHECK(SdramTiming::cyclesNeeded(7, SdramTiming::TrcdMinPs) == 3);  // 129.6
}
...
TEST_CASE("a count below the register's floor is reported as two clocks")
{
    ...
    CHECK(SdramTiming::cyclesNeeded(1, 1000) == 2);
    CHECK(SdramTiming::registerForCycles(2) == 0);
    CHECK(SdramTiming::registerForCycles(5) == 3);
}
...
TEST_CASE("cycle counts the two bits cannot hold are refused, not truncated")
{
    ...
    CHECK(SdramTiming::registerForCycles(6) == SdramTiming::NotEncodable);
    CHECK(SdramTiming::registerForCycles(1) == SdramTiming::NotEncodable);
}

TEST_CASE("the round trip through the register encoding is lossless")
{
    for (uint8_t reg = 0; reg < 4; ++reg) {
        CHECK(SdramTiming::registerForCycles(SdramTiming::cyclesFor(reg)) == reg);
    }
}
...
TEST_CASE("the precharge count is checked, not just the activate count")
{
    ...
    CHECK_FALSE(SdramTiming::inSpec(3, ThreeClocks, 0));
    CHECK_FALSE(SdramTiming::inSpec(3, 0, ThreeClocks));
}
