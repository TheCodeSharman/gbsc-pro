...
public:
    ...
    // wrote, and the hsync interval measured off the source. `separated` is
    // whether the source's hsync is regenerated by the sync separator, which
    // moves this counter's origin along the line.
    VideoSourceLine capturableLine(const HsyncPulse &pulse, bool separated) const;
