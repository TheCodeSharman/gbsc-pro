...
const uint16_t VideoSignal::RateTolerancePerMille;
...
bool VideoSignal::ratesAgree(uint32_t a, uint32_t b)
{
    const uint32_t larger = a > b ? a : b;
    const uint32_t smaller = a > b ? b : a;
    return (larger - smaller) * 1000u <= (uint32_t)RateTolerancePerMille * smaller;
}
