...
**`usableExtent` is the denominator, and it is not the input line.**
`MinimumCapture` bounds `extent` from below, for the reason it does today: a
control that can crop to nothing is one keypress from a dead picture.
