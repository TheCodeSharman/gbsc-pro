...
mode is judged from the count, the divider follows the scan mode and the rate,
and the duty is counted against that divider. Two facts, two moments.
...
because a count taken through the previous mode's divider is not the source's:
    ...
    event  ->  measure the count and the rate through whatever is in force
           ->  put the chip on the clock that rate asks for  <- the handoff
           ->  read the duty through it, and solve from the reading
...
`VideoPath::prepareToMeasure()` used to apply a reference sampling clock
...
`HD_HSYNC_RST` 2047, left third of the panel black. Pass-through now returns
before anything is installed, and `configurePassThrough()` holds the channel's
divider so the engine and the chip agree. Measured across a 1024x768 change
taken while passed through, the divider holds at 2039 and the picture is full
screen.
...
- The bring-up clock, `Adc::BringUpDivider` at `Adc::BringUpLineRateHz`, which
  `applyResetParameters()` leaves in force so that the state the chip is reset
  into can be measured through at all. Every divider after it is sized from a
  measured rate.
  `investigations/the-reference-divider-was-the-bootstrap.md`.
