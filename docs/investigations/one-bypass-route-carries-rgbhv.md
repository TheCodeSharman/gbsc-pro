...
### Both bands are CLOSED. Do not re-open them.

The left band is `HD_HS_ST`, corrected by the channel's own 40 counts; the right
is `HD_HB_ST`, moved off `applySd()`'s 0.945 of the line to the line's end. Each
was found by a single change and a fresh profile of BOTH edges, and each closes
its band completely rather than reducing it.

The long list of refuted registers above is what it cost to get there, and it is
kept so nobody sweeps them again. **It is not an open question**, and the
sentence it used to end on -- that the difference between the routes was not yet
accounted for -- described the state before `HD_HS_ST` was swept at all.

**A left-edge loss on the SCALING path is a different fault and is still open.**
...
bypass, and at 320x256@50 the bench display cannot show what comes out -- torn,
sheared content that reads as a broken scaling path. `docs/rgbhv-bypass-trap.md`
