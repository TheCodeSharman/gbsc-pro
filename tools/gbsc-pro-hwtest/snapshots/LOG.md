...
| `passthrough-akf50-clean-2026-09-09.json` | 09-09 | **pass-through with the colour clean, and the control for the colour flash.** RISC PC on the AKF50 mode file showing the DESKTOP, `STATUS_SYNC_PROC_VTOTAL` 627, full width, filer blues and folder icons correct. `DAC_RGBS_ADC2DAC` 1 with `BYPS2DAC` 0 and every matrix bypassed -- `HD_MATRIX_BYPS`, `HD_DYN_BYPS`, `DEC_MATRIX_BYPS` all 1, `RYSEL` 0/0/0, gains 123/123/123, offsets 64/64/64. **What makes it clean is the CONTENT**: the same build at the same raster with ModeServ's card up casts the whole picture red in one frame and green seconds later, and the desktop has no flashing border. **Trigger identified, still open** -- the source's border sets it off, which ModeServ flipped cyan and magenta, and red and green are their complements; `BORDER OFF` stops it dead. It is NOT the ADC clamp, which an earlier reading of this row proposed: moving the window from 19..108 to 250..350, out of the sync pulse and into the back porch, changed nothing. `docs/investigations/the-colour-flash-is-the-source-border.md`. `PLLAD_MD` 1856 = `HTOTAL`, `HPERIOD_IF` 176, `ADC_FLTR` 0, oversample 1, `SP_SOG_MODE` 0. On be58a24f4, so the flash was never a regression |
...
| `HSCALE-CORRUPT-osd-full-1536-2026-09-19.json` | 09-19 | **the zoom shear happening**, reached from the OSD pads alone with nothing hand-set. RiscPC 320x256@50, `VDS_HSCALE` 496, `VDS_HB_SP` 33 -- ODD, which is the fault: even is clean and odd shears, and the solve walks the register through both as the zoom steps. `VDS_DIS_HB` 140..1855, capture 189..1020, `PB_FETCH_NUM` 208. All 1536. `docs/known-issues.md` |
| `HSCALE-CORRUPT-full-1536-2026-09-19.json` | 09-19 | the same artefact but with `IF_HB_SP2`/`IF_HB_ST2` set BY HAND, so the capture window and the engine's framing disagree -- 900 units against an `eh` of 540. Kept as the state the `VDS_HB_SP` 8 -> 9 clearance was first seen at; not a reference for anything the engine solves. Paired with `HSCALE-CORRUPT-2026-09-19.dump.json`, 608 config + 48 status, restorable |
...
| `source-standard-ypbpr-576i-2026-08-26` | **the component branch of `Tv5725::SourceStandard`, proved on hardware.** The same Wii on YPbPr at PAL 576i as the row above, on the build where the standard ladder has left `doPostPresetLoadSteps()` entirely. Every field the standard decides is identical, the four a component source alone gets included -- `IF_HS_TAP11_BYPS` 0, `IF_HS_Y_PDELAY` 2, `VDS_V_DELAY` 0, `VDS_Y_DELAY` 3 -- so `inputIsYpBpR` now has a measurement behind it rather than only host tests. **23 fields differ against that row and none of them is the ladder.** Fourteen are the `HD_*` block, which `HdBypass::init()` writes at every arm and which reads 0 in a state that has not been armed since boot; `PA_ADC_BYPSZ`/`PA_SP_BYPSZ` are `togglePhaseAdjustUnits()`'s and `SP_HS_LOOP_SEL` is `SyncProcessor::applyForSyncType()`'s, all three history rather than configuration. `PA_SP_S` and `SP_H_CST_SP` are run-variable. The remaining four -- `VDS_HSYNC_RST`, `VDS_HSCALE`, `VDS_HB_ST`, `VDS_DIS_HB_ST` -- are `solveRaster()` against a field rate that moves, which is the wobble that set always shows. |

| `output-change-resolve-1080p-2026-08-27` | **the reference for what an output resolution change leaves**, and the settling of which oversampling is correct. 1080p reached from 480p by `/uc?s`, the engine re-solving from state it holds rather than a whole load: `PLLAD_CKOS` 0, `ADC_CLK_ICLK2X` 1, `DEC1_BYPS` 0, `PLLAD_MD` 2250 into a 1916 x 1125 raster, RiscPC 320x256@50, default framing. A whole load reaching the same resolution leaves a *different* oversampling group -- `CKOS` 1, `ICLK2X` 0, `DEC1_BYPS` 1 -- because it recomputes it from `PLLAD_KS` as the previous resolution left it. Both are internally consistent, both give a clean picture, and **this one is the baseline**: the engine derives it from the source's own measurement rather than from what the last raster happened to leave. Diff a re-solve against this, not against a load. `docs/investigations/an-output-change-is-not-a-source-change.md` |
