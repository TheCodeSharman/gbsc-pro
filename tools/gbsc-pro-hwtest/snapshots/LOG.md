...
| `passthrough-akf50-clean-2026-09-09.json` | 09-09 | **pass-through with the colour clean, and the control for the colour flash.** RISC PC on the AKF50 mode file showing the DESKTOP, `STATUS_SYNC_PROC_VTOTAL` 627, full width, filer blues and folder icons correct. `DAC_RGBS_ADC2DAC` 1 with `BYPS2DAC` 0 and every matrix bypassed -- `HD_MATRIX_BYPS`, `HD_DYN_BYPS`, `DEC_MATRIX_BYPS` all 1, `RYSEL` 0/0/0, gains 123/123/123, offsets 64/64/64. **What makes it clean is the CONTENT, not the timing**: the same build at the same 627 raster with ModeServ's PM5544 card casts the whole picture red in one frame and green seconds later. Those are the complements of the cyan and magenta ModeServ flashes the border, which is what a black reference taken from active video does. Clamp window `SP_CS_CLP_ST` 19..`SP_CS_CLP_SP` 108 of an 1856-sample line, `SP_CLAMP_MANUAL` 1. `PLLAD_MD` 1856 = `HTOTAL`, `HPERIOD_IF` 176, `ADC_FLTR` 0, oversample 1, `SP_SOG_MODE` 0. On be58a24f4, so the flash is not a regression |
