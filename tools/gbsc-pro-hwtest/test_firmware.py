...
# The leg that pays: the destination is line-doubled and the departure is not,
# so the divider in force describes the wrong scan mode and the count wanders
# until the engine rewrites it.
DOUBLING_LEG_DEPARTURE = "MODE X640 Y480 C256 F60"
DOUBLING_LEG_DESTINATION = "MODE X320 Y256 C256 F50"

# What the console prints when the engine arms a re-measure, and when one has
# produced a line rate worth holding.
SOURCE_MOVED = re.compile(r"source moved: [\w ]+ \((\d+) lines, solved (\d+)\)")
SOLVED = re.compile(r"sampling: (\d+) lines x [\d.]+ Hz -> line rate [1-9]")


@pytest.mark.source_mode
def test_one_source_mode_change_is_answered_by_one_re_measure(
    host, console, source, request
):
    """A mode change must arm on the move, not again on the count it solved.

    Every arm re-installs the reference sampling clock and blanks the output
    while it measures, so a second one on a source that has not moved costs the
    encoder another relock in the middle of the transition -- measured as seven
    frames of flash on this leg, with the console naming the count it had just
    solved:
    ...
    Two things latch a disturbance the engine then reads as a source event: the
    source's own mode change, and the sync-type probe's own write of
    SP_EXT_SYNC_SEL. Neither is news once the measurement that followed it has
    landed.

    THE COUNT MOVES ON THIS LEG, which is what makes an arm reporting the count
    it solved a fault rather than the case the interrupt exists for -- a source
    returning at the SAME count and a different field rate, which no count can
    see. 320x256 at 50, 55 and 60 are that case and this is not.
    ...
    lines = console.collect(SOURCE_SETTLE_SECONDS)
    assert console.alive, "the console died mid-capture; this run proves nothing"

    assert any(SOLVED.search(line) for line in lines), (
        f"no measurement reached the console across {DOUBLING_LEG_DESTINATION}, "
        f"so nothing here is evidence about the arms. Captured: {lines!r}"
    )

    standing = [
        line for line in lines
        for m in [SOURCE_MOVED.search(line)]
        if m and m.group(1) == m.group(2)
    ]
    assert not standing, (
        f"the engine armed a re-measure on a count it had already solved: "
        f"{standing}. Each arm blanks the output and re-installs the reference "
        "sampling clock, so the encoder relocks twice on one mode change."
    )
