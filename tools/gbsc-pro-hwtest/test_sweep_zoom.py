"""Unit tests for the keypress-driven capture sweep. No hardware needed."""
...
class FakeUnit:
    """A unit whose capture runs exactly as far as the extent asks.

    That is what the engine does -- the extent IS the capture width in input
    units -- so a fake that ignored it could not tell a sweep that steps from
    one that stands still, which is the property most of these tests are about.
    """

    def __init__(self, extent=843, clamp=None):
        self.extent = extent
    ...
    def apply(self, value):
        ...
        return self.snapshot(), self.extent

    def snapshot(self):
        width = self.extent if self.clamp is None else min(self.extent, self.clamp)
...
def sweep(unit, step=1, prefix="dense", first=1, recorded=None):
    return sweep_zoom.Sweep(unit.apply, unit.snapshot, (recorded if recorded
                            ...
                            extent=unit.extent, step=step, prefix=prefix, first=first)
...
def test_a_step_moves_the_extent_by_one_unit_and_measures_there():
    unit = FakeUnit()
    s = sweep(unit, step=1)

    s.step()

    assert unit.applied == [844]
...
def test_the_extent_travels_with_the_row():
    ...
    s = sweep(unit, step=1)
    s.step()
    assert s.pending["framing"] == {"axis": "h", "extent": 844}
...
def test_stepping_back_returns_to_the_previous_extent_and_measures_again():
    ...
    s.step()
    s.back()
    assert unit.applied == [844, 845, 844]
    assert s.pending["registers"]["IF_HB_ST2"] == BENCH["IF_HB_SP2"] + 844
...
def test_a_press_judges_the_step_and_walks_on_to_the_next():
    ...
    assert unit.applied == [844, 845, 846]
...
def test_the_line_on_screen_describes_the_axis_being_swept():
    # The vertical sweep is the next experiment -- does the null move when only
    ...
    row = characterise.observe(dict(BENCH, IF_VB_SP=48, IF_VB_ST=576).get,
                               "x", "clean")
    row["framing"] = {"axis": "v", "extent": 528}
    line = sweep_zoom.describe(row)
    assert "VSCALE 483" in line
    assert "48..576" in line


def test_a_row_recorded_against_the_old_zoom_coordinate_still_prints():
    # The tracked characterisation index is 661 rows deep in the coordinate the
    # framing used before it became an origin and an extent. Reading them has to
    # keep working, and has to SAY which coordinate it is showing: the two count
    # in opposite directions.
    row = characterise.observe(BENCH.get, "x", "clean")
    row["framing"] = {"axis": "h", "zoom": 4}

    assert "zoom     4" in sweep_zoom.describe(row)
...
def test_a_sweep_can_start_somewhere_other_than_where_the_unit_is():
    ...
    unit = FakeUnit(extent=667)
    s = sweep_zoom.Sweep(unit.apply, unit.snapshot, [].append,
                         extent=1000, step=1, restore_to=unit.extent)

    s.step()

    assert unit.applied == [1001]
...
def test_restoring_goes_back_to_the_unit_not_to_the_start_point():
    ...
    unit = FakeUnit(extent=667)
    s = sweep_zoom.Sweep(unit.apply, unit.snapshot, [].append,
                         extent=1000, step=1, restore_to=667)
    s.step()
    s.restore()
    assert unit.applied[-1] == 667
